...
import { CartPropiasManagementCycleModule } from './interfaces/modules/cartPropiasManagementCycle.module';
  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => {
        const schema = config.get<string>('SCHEMA') as 'mysql' | 'postgres';
        const defaultPort = schema === 'postgres' ? 5432 : 3306;
        return {
        type: schema,
        host: config.get('DB_CONFIG_HOST', 'localhost'),
        port: config.get<number>('DB_CONFIG_PORT', defaultPort),
    ...
    CartPropiasManagementCycleModule,
    LogsModule,
  ],
})

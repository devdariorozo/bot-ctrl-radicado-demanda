...
export class TblPortfolioTypeMigration1771978729003 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    if (isPostgres(queryRunner)) {
      await queryRunner.query(`DROP TABLE IF EXISTS tbl_portfolio_type CASCADE`);
      await queryRunner.query(`
        CREATE TABLE tbl_portfolio_type (
          porty_id     SERIAL       PRIMARY KEY,
          porty_type   VARCHAR(20)  NOT NULL,
          porty_detail VARCHAR(100) NOT NULL,
          porty_state_type_id INT   NOT NULL,
          porty_created_at  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
          porty_updated_at  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

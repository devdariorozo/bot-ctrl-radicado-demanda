...
export class AlterAutomationEmailDateReceivedToVarchar1771978729020 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    if (isPostgres(queryRunner)) {
      const rows: unknown[] = await queryRunner.query(
        `SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'tbl_automation_email' LIMIT 1`,
      );
      if (!Array.isArray(rows) || rows.length === 0) return;
      await queryRunner.query(`
        ALTER TABLE tbl_automation_email
        ALTER COLUMN autm_date_received TYPE VARCHAR(255) USING autm_date_received::varchar
      `);
    } else {
      const rows: unknown[] = await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    if (isPostgres(queryRunner)) {
      await queryRunner.query(`
        ALTER TABLE tbl_automation_email
        ALTER COLUMN autm_date_received TYPE TIMESTAMP USING autm_date_received::timestamp
      `);
    } else {
      await queryRunner.query(`
        ALTER TABLE tbl_automation_email
